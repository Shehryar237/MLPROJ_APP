...
# Update class labels based on your SVM model
CLASS_LABELS = ["Healthy", "Moderate", "Severe"]

# Load SVM model and scaler
def load_model():
    global svm_model, scaler
    model_path = '.\model\svm_model.joblib'  # Path to your saved SVM model
    scaler_path = '.\model\scaler.joblib'    # Path to your saved scaler

    try:
        if not os.path.exists(model_path):
            print(f"SVM model file does not exist at the specified path: {model_path}")
            svm_model = None
            return
        ...
        print(f"Loading SVM model from {os.path.abspath(model_path)}")
...
def preprocess_image_svm(image_path, image_size=(128, 128)):
    """
    ...
    try:
        ...
        print(f"Error in SVM image preprocessing: {e}")
        return None
...
def extract_features_svm(image):
    ...
    try:
        ...
        print(f"Error in SVM feature extraction: {e}")
        return None
...
def predict():
    ...
    try:
        ...
        preprocessed_image = preprocess_image_svm(file_path)
        if preprocessed_image is None:
            print("Error processing the image during preprocessing")
            return jsonify({'error': 'Error processing image'}), 500

        # Extract features
        features = extract_features_svm(preprocessed_image)
        if features is None:
...
def health():
    if svm_model is not None and scaler is not None:
        return jsonify({'status': 'SVM model and scaler loaded successfully and service is healthy'}), 200
    else:
...
if __name__ == '__main__':
    if svm_model is not None and scaler is not None:
        ...
        print("Failed to load the SVM model or scaler. Exiting.")
